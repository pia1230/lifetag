...
import { forceGlobalRefresh } from "../utils/dataRefresh";
...
const PatientRequests = () => {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      ...
      setError(null);
      console.log('Patient requests refreshed:', pending.length, 'pending,', active.length, 'active');
    } catch (err) {
  ...
  useEffect(() => {
    fetchRequests();
    // Set up interval to poll for new data every 3 seconds
    const intervalId = setInterval(fetchRequests, 3000);
    
    // Listen for custom refresh events
    const handleForceRefresh = () => {
      console.log('Force refresh triggered for patient requests');
      fetchRequests();
    };
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      // Force immediate global refresh
      forceGlobalRefresh('requests');
    } catch (err) {
